...
cuis=[]
morecui=[]
METAMAP="../public_mm/bin/metamap18"
chatstep = 0
SLACK_SIGNING_SECRET =
SLACK_BOT_TOKEN =

#dictionary for conversion of concept code to text
f = open("../2019AB/META/MRCONSO.RRF",'r', encoding='utf-8')
l = f.readline()
...
csp={}
nci={}
f = open("../2019AB/META/MRCONSO.RRFMRDEF.RRF",'r', encoding='utf-8')
l = f.readline()
for i in range(10000):
	while (l):
		words = l.strip().split("|")
...
app = Flask(__name__)
#Slack Events Adapter is a Python-based solution to receive and parse events from Slack’s Events API. Before you can use the Events API you must create a Slack App, and turn on Event Subscriptions.
...
def predictSpecialist(cuilist):
	...
	testdf[cuilist]=1
	logisticRegr = pickle.load(open("../specialist_classifier_logReg_UMLS/">,'rb'))
...
def findMoreSymptoms(cuilist):
	df = pd.read_csv("../discipline_symptoms_umls2.csv/",header=None)
...
def start_onboarding(user_id: str, channel: str):
	# Create a new onboarding msg.
	onboarding_msg = ChatBot(channel)

	# Get the onboarding message payload
	message = onboarding_msg.get_message_onboarding()
	...
	onboarding_msg.timestamp = response["ts"]

	# Store the message sent in onboarding_msg_sent
	if channel not in onboarding_msg_sent:
		onboarding_msg_sent[channel] = {}
	onboarding_msg_sent[channel][user_id] = onboarding_msg
...
def update_emoji(payload):
	...
	if channel_id not in onboarding_msg_sent:
		return

	# Get the original  sent.
	onboarding_msg = onboarding_msg_sent[channel_id][user_id]

	# Mark the reaction task as completed.
	onboarding_msg.reaction_task_completed = True

	# Get the new message payload
	message = onboarding_msg.get_message_payload()

	# Post the updated message in Slack
	updated_message = slack_web_client.chat_update(**message)

	# Update the timestamp saved on the onboarding msg object
	onboarding_msg.timestamp = updated_message["ts"]
...
def update_pin(payload):
	...
	# Get the original msg sent.
	onboarding_msg = onboarding_msg_sent[channel_id][user_id]

	# Mark the pin task as completed.
	onboarding_msg.pin_task_completed = True
